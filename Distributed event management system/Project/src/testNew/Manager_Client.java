...
public class Manager_Client implements Runnable{
	...
	public Manager_Client(String managerId, String userChoice, String eventType, String eventId, String capacity, String eventType_n, String eventId_n) {
	...
	public Boolean checkServerInfo(String n_manaID) {
		...
		int serverPort = 0;
		// set the server port
		if (this.managerId.substring(0, 3).equalsIgnoreCase("tor")) {
			serverPort = SERVER_PORT_TOR;
		} else if (this.managerId.substring(0, 3).equalsIgnoreCase("mtl")) {
			serverPort = SERVER_PORT_MTL;
		} else if (this.managerId.substring(0, 3).equalsIgnoreCase("otw")) {
...
//			System.out.println("Please input the manager ID:");
			...
			while (!valid1) {
				id = this.managerId.toUpperCase().trim();
			...
			if (!checkServerInfo(this.managerId)) {
				System.out.println("Manager ID is not exist, please try again!");
			} else {
				System.out.println(this.managerId + " log in successfully!");
				valid0 = true;
			}
		}

		Server_interface serverobj = getReference(this.orb, managerId.substring(0, 3));
		initLogger(this.managerId);
		LOGGER.info("Request: " + this.managerId + " log in DEMS System.");

		try {
			String userChoice = "";
//			showMenu(this.managerId);
//			while (true) {
				Boolean valid = false;
				while (!valid) {
...
//					System.out.println("Please add bookingCapacity:");
					...
					while (!capvalid1) {
						e_capacity1 = this.capacity.trim();
						if (e_capacity1.matches("^\\d+$") && Integer.parseInt(e_capacity1) > 0)
							capvalid1 = true;
						else
							System.out.println("Invalid capacity input, please enter an valid Integer!");
					}
...
//					System.out.println("Please input eventID:");
					...
					while (!eidvalid2) {
						e_id2 =this.eventId.toUpperCase().trim();
...
//					System.out.println("Please choose eventType:\n1. Conferences\n2. Seminars\n3. Trade Shows");
					...
					LOGGER.info("Request: remove event " + getEventName(e_type2) + " " + e_id2 + ". Response: " + s2);
//					showMenu(this.managerId);
					break;
...
//					System.out.println("Please choose eventType:\n1. Conferences\n2. Seminars\n3. Trade Shows");
					...
					LOGGER.info("Request: list event availability " + getEventName(e_type3) + ". Response: " + s3);
//					showMenu(this.managerId);
					break;
...
//					System.out.println("Please input customer ID:");
					...
					while (!cidvalid4) {
						c_id4 = this.eventId.toUpperCase().trim();
...
//					System.out.println("Please input eventID:");
					...
					while (!eidvalid4) {
						e_id4 = this.eventId.toUpperCase().trim();
...
//					System.out.println("Please input customer ID:");
					...
					while (!cidvalid5) {
						c_id5 = this.eventId.toUpperCase().trim();
...
//					System.out.println("Please input customer ID:");
					...
					while (!cidvalid6) {
						c_id6 = this.eventId.toUpperCase().trim();
...
//					System.out.println("Please input eventID:");
					...
					if (!eidvalid6) {
						e_id6 = this.eventId.toUpperCase().trim();
...
//					System.out.println("Please input customer ID:");
					...
					while (!cidvalid7) {
						c_id7 = this.managerId.toUpperCase().trim();
...
//					System.out.println("Please input old eventID:");
					...
					if (!eidvalid7_o ) {
						e_id7_o  = this.eventId.toUpperCase().trim();
...
//					System.out.println("Please choose new eventType:\n1. Conferences\n2. Seminars\n3. Trade Shows");
					...
					while (!etypevalid7_n ) {
						e_type7_n  = this.eventType_n.trim();
...
//					System.out.println("Please input new eventID:");
					...
					if (!eidvalid7_n ) {
						e_id7_n  = this.eventId_n.toUpperCase().trim();
